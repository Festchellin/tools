...
# install git zsh wget curl net-tools
sudo apt-get install git zsh wget curl net-tools -y

# config the git username and email
git config --global user.name "Festchellin"

git config --global user.email "festchellin@outlook.com"
echo '
#!/bin/sh
#
	...
	if [ -f ~/.zshrc ] || [ -h ~/.zshrc ]; then
		...
		if [ -e "$OLD_ZSHRC" ]; then
			...
			if [ -e "$OLD_OLD_ZSHRC" ]; then
				error "$OLD_OLD_ZSHRC exists. Cant back up ${OLD_ZSHRC}"
...
setup_shell() {
	...
	# If this users login shell is already zsh, do not attempt to switch.
	if [ $(basename $SHELL) = zsh ]; then
		return
	fi

	# If this platform doesnt provide a "chsh" command, bail out.
	if ! command_exists chsh; then
		cat <<-EOF
			I cant change your shell automatically because this system does not have chsh.
	...
	# Check if were running on Termux
	case "$PREFIX" in
	...
	if [ "$termux" != true ]; then
		...
		# 1. Use the most preceding one based on $PATH, then check that its in the shells file
		# 2. If that fails, get a zsh path from the shells file, then check it actually exists
		if ! zsh=$(which zsh) || ! grep -qx "$zsh" "$shells_file"; then
			if ! zsh=$(grep ^/.*/zsh "$shells_file" | tail -1) || [ ! -f "$zsh" ]; then
				error "no zsh binary found or not present in "
	...
	# Were going to change the default shell, so back up the current one
	if [ -n "$SHELL" ]; then
...
main() {
	...
	if [ -d "$ZSH" ]; then
		cat <<-EOF
			...
			Youll need to remove "$ZSH" if you want to reinstall.
		EOF
		exit 1
	fi
